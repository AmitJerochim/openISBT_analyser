#!/bin/bash
file=../loggings/determine_supported_operations.log
LOG_FILES=../log_files/*
SUM_CREATE=0
SUM_READ=0
SUM_SCAN=0
...
controls_in_file=$(cat $f |grep 'mapping.Mapper' | grep 'Operation' | grep ' CONTROL ' | wc -l)
infos_in_file=$(cat $f |grep 'mapping.Mapper' | grep 'Operation' | grep 'Operation: INFO ' | wc -l)
...
SUM_CONTROL=$((SUM_CONTROL+controls_in_file))
SUM_INFO=$((SUM_INFO+infos_in_file))
done
...
echo -e "NUMBER CONTROL\t$SUM_CONTROL"
echo -e "NUMBER INFO\t$SUM_INFO"

All_OPERATIONS=$(cat $file | grep "MEASUREMENT SUMMARY:"| grep "all available operations ignoring" | sed "s/MEASUREMENT SUMMARY.*ignoring operations on sub-resource:\t\t//")
UNDEFINED=$((All_OPERATIONS-SUM_CREATE-SUM_READ-SUM_SCAN-SUM_UPDATE-SUM_DELETE-SUM_AUTHENTICATE-SUM_VALIDATE-SUM_OPTIONS-SUM_INFO))

echo -e "NUMBER UNDEFINED\t$UNDEFINED"
